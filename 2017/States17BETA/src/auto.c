...
void auto1() {
  ...
  driveSet(-127, -127);
  TaskHandle liftToTask1 = taskCreate(liftToTask, TASK_DEFAULT_STACK_SIZE, (void *)arr, TASK_PRIORITY_DEFAULT);
  while (digitalRead(isWall) == 1 && digitalRead(isWall2) == 1 && timer(0) < 3000) { // Repeats until the button on the robot is pressed or 4 seconds pass
    delay(1);
  }
  driveStop();
}
...
void auto0() {
  ...
  turn(88, 70);        // Turns 83 degrees to the right at 40 power
