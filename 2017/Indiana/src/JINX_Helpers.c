...
void sendValues(void) {
  if (USE_JINX) {
    ...
    char rightDrive[50];
    */
    sprintf(leftQuad, "%d", encoderGet(lencoder));
    sprintf(rightQuad, "%d", encoderGet(rencoder));
    /*
    sprintf(liftPot, "%d", analogReadCalibrated(POT));
    sprintf(gyroValue, "%d", gyroGet(gyro));
    sprintf(leftDrive, "%d", motorGet(TLD));
    sprintf(rightDrive, "%d", motorGet(TRD));
    */
    mutexGive(potMutex);
    mutexGive(driveMutex);
    writeJINXData("Left Quad", leftQuad);
    writeJINXData("Right Quad", rightQuad);
    /*
    writeJINXData("Lift", liftPot);
    writeJINXData("Gyro", gyroValue);
    writeJINXData("Left Drive Power", leftDrive);
    writeJINXData("Right Drive Power", rightDrive);
    */
  }
}
